...
```javascript
<div className="className="h-4 w-4 rounded border-gray-300 text-indigo-500 focus:ring-indigo-300"></div>
```
...
> 사실 이 문제를 해결하기 위해서 사이드바 자체의 너비 조절, 텍스트 크기 수정, overflow 사용 등등 도전했다. 구조를 바꾸는 것은 최후의 수단으로 남겨두었다. 계속 두드려 보고 시도해 봐서 마침내 성공했다. `fade-in-out `애니메이션도 도전하는 것이 목표. 다수의 코드 작성이 아닌 한 단어로 수정하는 기쁨 ☺️
